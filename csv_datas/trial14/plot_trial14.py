...
def plot_joint_data(file_number, start_time=None, end_time=None):
    ...
    axs[2, 0].plot(df["current_time"].to_numpy(), df["estimated_disturbance_joint1"].to_numpy(), label="Estimated Disturbance (J1)", linestyle='--', linewidth = 2.5)
    axs[2, 0].plot(df["current_time"].to_numpy(), df["saturated_DOB_joint1"].to_numpy(), label="saturated DOB input (J1)", linestyle=':', linewidth = 4.5)
